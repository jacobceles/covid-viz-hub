...
""" racing bar"""
confirmed_global_df_grouped = confirmed_global_df.groupby(['country', 'time_period']).sum().reset_index()
confirmed_global_race_plot = barplot(confirmed_global_df_grouped, item_column='country',
...
recovered_choropleth = px.choropleth(recovered_global_df,
                                     ...
                                     animation_frame="time_period",
                                     color_continuous_scale='agsunset',
                                     labels={'time_period': 'Month', 'recovered': 'Recovered Count',
...
layout = html.Div([
    dbc.Container([
        ...
        dbc.Row([dbc.Col(html.H5(children='Rate of spread in the top 10 countries',
                                 className="text-center"), className="mt-4"), ]),
        dbc.Row([html.Div([
            dcc.Graph(figure=confirmed_global_race_plot.plot(item_label='Countries',
                                                             value_label='Confirmed (cumulative)',
                                                             frame_duration=600))])]),
    ])
])
