...
def confirmed_us(df):
    ...
    df = replace_character_in_column_names(df, "/", "_")
    df = delete_columns(df, ['uid', 'iso2', 'iso3', 'code3', 'fips', 'country_region', 'lat', 'long_', 'combined_key'])
    df = rename_date_columns(df, ok_columns_set, '_')
    df = convert_to_month_wise_df(df, {'admin2', 'province_state', 'population'}, '/')
    df = melt_columns_to_rows(df, ['admin2', 'province_state', 'population'], "time_period", "cumulative_confirmed")
    df = unroll_cumulative_sum(df, ['admin2', 'province_state', 'cumulative_confirmed'], ['admin2', 'province_state'])
...
def confirmed_us_normalized(df):
    ...
    new_df = confirmed_us_states.join(population_us_states).reset_index()
    new_df = new_df.loc[:, ~new_df.columns.duplicated()]
    new_df = new_df.T.drop_duplicates().T
    new_df = new_df.astype({"cumulative_confirmed": int, "population": int, })
    confirmed_us_states.reset_index(level=0, inplace=True, drop=True)
    population_us_states.reset_index(level=0, inplace=True, drop=True)
    new_df['confirmed_percent'] = new_df['cumulative_confirmed'] / new_df['population']
    return new_df


# merging population column from death_us_df to confirmed_us_df
def extract_and_marge(deaths_us_source_df, confirmed_us_source_df):
    deaths_us_df_rd = pd.read_csv(deaths_us_source_df)
    deaths_us_df_rd.columns = map(str.lower, deaths_us_df_rd.columns)
    confirmed_us_df_rd = pd.read_csv(confirmed_us_source_df)
    ...
    return pd.merge(confirmed_us_df_rd, population_us_df_rd,
                    on=['province_state', 'country_region', 'lat', 'long_', 'admin2', 'uid'], how='inner')
